	...
	public class ChmHelp
	{
		...
		private void Create(string path)
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				stringBuilder.AppendLine("\t\t<param name=\"Name\" value=\"{0}\">".FormatString(new object[]
				{
					Path.GetFileNameWithoutExtension(text)
				}));
				stringBuilder.AppendLine("\t\t<param name=\"Local\" value=\"{0}\">".FormatString(new object[]
				{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				this.hhcBody.AppendLine("\t\t<param name=\"Name\" value=\"{0}\">".FormatString(new object[]
				{
		...
		private void CreateHHP()
		{
			...
			File.WriteAllText(Path.Combine(this.SourcePath, "chm.hhp"), stringBuilder.ToString().FormatString(new object[]
			{
				this.Title,
				this.DefaultPage
		...
		public bool Compile()
		{
			...
			try
			{
				process.StartInfo = new ProcessStartInfo
				{
					...
					FileName = text,
					Arguments = "\"{0}\"".FormatString(new object[]
					{
