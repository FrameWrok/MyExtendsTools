...
namespace System.Security
{
    ...
    public static partial class Encrypt
    {
        ...
        public static string EncryptString(this string input, out string key, out string iv, EncryptSymmetryType encryptSymmetryType)
        {
            string result;
            string keY;
            string iV;
        ...
        /// <param name="encryptSymmetryType">要采用的对称加密算法</param>        
        /// <returns>加密后的结果</returns>
        public static byte[] EncryptString(this string input, out byte[] key, out byte[] iv, EncryptSymmetryType encryptSymmetryType)
        {
            byte[] resultBuffer;
            byte[] keyBytes;
            byte[] ivandBytes;
        ...
        /// <param name="encryptSymmetryType">要采用的对称加密算法</param>        
        /// <returns>加密后的结果</returns>
        private static string EncryptString(this string input, out string key, out string iv, int bitLength, EncryptSymmetryType encryptSymmetryType)
        {
            byte[] resultBuffer;
            byte[] keyBytes;
            byte[] ivandBytes;
        ...
        /// <param name="encryptSymmetryType">要采用的对称加密算法</param>        
        /// <returns>加密后的结果</returns>
        private static byte[] EncryptString(this string input, out byte[] key, out byte[] iv, int bitLength, EncryptSymmetryType encryptSymmetryType)
        {
        ...
        /// <param name="encryptSymmetryType">要采用的对称加密算法</param>
        /// <returns>加密后的结果</returns>
        private static string EncryptStringByKeyIV(this string input, string key, string iv, EncryptSymmetryType encryptSymmetryType)
        {
        ...
        /// <param name="encryptSymmetryType">要采用的对称加密算法</param>
        /// <returns>加密后的结果</returns>
        private static string EncryptStringByKeyIV(this string input, string key, string iv, int bitLength, EncryptSymmetryType encryptSymmetryType)
        {
            if (input == null || input.Length <= 0)
                throw new ArgumentNullException("input");
        ...
        /// <param name="encryptSymmetryType">要采用的对称加密算法</param>
        /// <returns>加密后的结果</returns>
        private static byte[] EncryptStringByKeyIV(this string input, byte[] key, byte[] iv, EncryptSymmetryType encryptSymmetryType)
        {
            return EncryptStringByKeyIV(input, key, iv, int.MinValue, encryptSymmetryType);
        }
    ...
    public static partial class Encrypt
    {
        ...
        public static string EncryptStringHash(this string input, EncryptHashType encryptHashType)
        {
            if (input == null || input.Length <= 0)
                throw new ArgumentNullException("input");
    ...
    public static partial class Encrypt
    {
        ...
        public static string DSASignaturetStringAsymmetric(this string input, out string publicKeyString, out string privateKeyString)
        {
        ...
        public static string DSASignaturetStringAsymmetricByPrivateKey(this string input, string privataKeyXmlString)
        {
            byte[] rgbHash = (new SHA1CryptoServiceProvider()).ComputeHash(Encoding.Default.GetBytes(input));
            byte[] resultBytes;
            using (DSACryptoServiceProvider newDSA = new DSACryptoServiceProvider())
        ...
        public static string SignaturetStringAsymmetric(this string input, out string publicKeyString, out string privateKeyString, EncryptAsymmetricType encryptAsymmetricType)
        {
            publicKeyString = string.Empty;
            privateKeyString = string.Empty;
            byte[] rgbHash = (new SHA1CryptoServiceProvider()).ComputeHash(Encoding.Default.GetBytes(input));
            byte[] resultBytes;
            AsymmetricAlgorithm cryptoServiceProvider = null;
            AsymmetricSignatureFormatter asymmetricSignatureFormatter = null;
            switch (encryptAsymmetricType)
            {
                case EncryptAsymmetricType.DSA:
                    {
                        cryptoServiceProvider = new DSACryptoServiceProvider() { PersistKeyInCsp = false };
                        asymmetricSignatureFormatter = new DSASignatureFormatter(cryptoServiceProvider);
                    } break;
                case EncryptAsymmetricType.RSA:
                    {
                        cryptoServiceProvider = new RSACryptoServiceProvider() { PersistKeyInCsp = false };
                        asymmetricSignatureFormatter = new RSAPKCS1SignatureFormatter(cryptoServiceProvider);
                    } break;
            }
            publicKeyString = cryptoServiceProvider.ToXmlString(false);
            privateKeyString = cryptoServiceProvider.ToXmlString(true);
            asymmetricSignatureFormatter.SetHashAlgorithm("SHA1");
            resultBytes = asymmetricSignatureFormatter.CreateSignature(rgbHash);
            return Convert.ToBase64String(resultBytes);
        }
        ...
        /// <summary>
        /// 根据输入的xml公钥返回用DSA进行加密的数据(采用SHA1散列加密的数据)
        /// </summary>
        /// <param name="input">加密字符串</param>
        /// <param name="publicKeyString">公钥</param>
        /// <returns>解密后的内容</returns>
        public static string RSASignaturetStringAsymmetric(this string input, string publicKeyString)
        {
            byte[] rgbHash = (new SHA1CryptoServiceProvider()).ComputeHash(Encoding.Default.GetBytes(input));
            byte[] resultBytes;
    ...
    public static partial class Encrypt
    {
        ...
        internal static SymmetricAlgorithm GetSymmetricAlgorithmBySmartEncryptSymmetryType(EncryptSymmetryType encryptHashType)
        {
            SymmetricAlgorithm symmetricAlgorithm = null;
            switch (encryptHashType)
            {
                case EncryptSymmetryType.Aes:
                    {
                        symmetricAlgorithm = AesCryptoServiceProvider.Create();
                    }

                    break;
                case EncryptSymmetryType.AesManaged:
                    {
        ...
        private static HashAlgorithm GetHashAlgorithmBySmartEncryptHashType(EncryptHashType smartEncryHashType)
        {
            HashAlgorithm hashAlgorithm = null;
            switch (smartEncryHashType)
            {
                case EncryptHashType.MD5:
                    {
                        hashAlgorithm = new MD5CryptoServiceProvider();
                    }

                    break;
                case EncryptHashType.SHA1:
                    {
                        hashAlgorithm = new SHA1CryptoServiceProvider();
                    }

                    break;
                case EncryptHashType.SHA256:
                    {
                        hashAlgorithm = new SHA256Managed();
                    }

                    break;
                case EncryptHashType.SHA384:
                    {
                        hashAlgorithm = new SHA384Managed();
                    }

                    break;
                case EncryptHashType.SHA512:
                    {
